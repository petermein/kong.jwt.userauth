...
local get_key_for_value = utils.get_key_for_value
...
-- @return A boolean indicating true if no errors zere found
-- @return A list of errors
function _M:verify_scopes(scopes_to_verify, scope_claim)
  if not scopes_to_verify then
    scopes_to_verify = {}
  end
  ...
  for _, scope_name in pairs(scopes_to_verify) do
    scopes = self.claims[scope_claim]

    if get_key_for_value(scopes, scope_name) == nil then
      return false, { validate_scopes = scope_name .. " is required" } 
    end
  end
